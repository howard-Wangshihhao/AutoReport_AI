...
"""依 OWASP MASTG Android / iOS Excel 的「判定結果 / 目前情況」產生雙語資安報告。

特色：
- Android / iOS 共用同一支程式，自動依工作表名稱辨識平台。
- Android 僅處理共通測項 S/D/F 與 MAxx；iOS 僅處理共通測項 S/D/F 與 MIxx。
- 可解析 Excel HYPERLINK() 測項公式，只把顯示的 MASTG 測項名稱送給 AI。
- 保留 Report / Report_ch / CVSS / score、CVSS retry、N/A、紅字 mismatch 與 Security Recommendation。

使用方式：
    python generate_mastg_report_ai.py check_android_ai.xlsx
    python generate_mastg_report_ai.py check_ios_ai.xlsx
...
10. 這是 OWASP MASTG 行動 App 測試。平台會明確提供為 Android 或 iOS；只使用該平台適用的術語，不得把另一平台特有機制混入報告。
11. MASTG 測項名稱只代表「要檢查什麼」，不是已觀察到的漏洞證據；弱點、影響、攻擊條件與既有防護仍必須以「目前情況」為依據。
12. 僅輸出一個 JSON object，不要 Markdown、code fence、前言或解釋。
...
REPORT_RESPONSE_SCHEMA = {
    ...
    "properties": {
        ...
        "cvss": {"anyOf": [CVSS_METRICS_SCHEMA, {"type": "null"}]},
    },
...
HYPERLINK_DISPLAY_RE = re.compile(
    r'^=HYPERLINK\(\s*"(?:[^"]|"")*"\s*,\s*"((?:[^"]|"")*)"\s*\)$',
    re.IGNORECASE,
)


def extract_test_item_text(value) -> str:
    """取得測項顯示文字；若儲存格是 HYPERLINK() 公式，回傳第二參數的顯示名稱。"""
...
def extract_masvs_domain(value) -> str:
    """從 MASTG hyperlink/path 或標題中找出 MASVS-STORAGE 等 domain。"""
    match = re.search(r'\b(MASVS-[A-Z][A-Z-]*)\b', clean_text(value), re.IGNORECASE)
    return match.group(1).upper() if match else ""


def detect_platform_and_sheet(wb, requested_platform: str = "auto") -> tuple[str, str]:
    """依 --platform 或工作表名稱辨識 Android / iOS。"""
    requested = clean_text(requested_platform).casefold() or "auto"
    candidates = {
        "android": "Android App" if "Android App" in wb.sheetnames else None,
        "ios": "iOS App" if "iOS App" in wb.sheetnames else None,
    }
    if requested in {"android", "ios"}:
        sheet = candidates[requested]
        if not sheet:
            raise ValueError(f"指定平台 {requested}，但找不到對應工作表：{'Android App' if requested == 'android' else 'iOS App'}")
        return requested, sheet
    if requested != "auto":
        raise ValueError("platform 僅支援 auto、android、ios")

    found = [(platform, sheet) for platform, sheet in candidates.items() if sheet]
    if len(found) == 1:
        return found[0]
    if not found:
        raise ValueError("無法辨識平台：找不到 'Android App' 或 'iOS App' 工作表")
    raise ValueError("同時找到 Android App 與 iOS App，請使用 --platform 指定平台")


def is_relevant_item_id(platform: str, item_no: str) -> bool:
    """Android: S/D/F + MAxx；iOS: S/D/F + MIxx。"""
    item = clean_text(item_no).upper()
    if re.fullmatch(r"[SDF]\d+", item):
        return True
    if platform == "android":
        return bool(re.fullmatch(r"MA\d+", item))
    if platform == "ios":
        return bool(re.fullmatch(r"MI\d+", item))
    return False


def find_last_content_row(ws, header_row: int, headers: dict[str, int]) -> int:
    """避免模板尾端大量格式化空白列被無謂處理。"""
    cols = [headers.get(name) for name in ("分類", "編號", "測項", "判定結果", "目前情況", "Report", "Report_ch")]
    cols = [c for c in cols if c]
    last = header_row
    for row_idx in range(header_row + 1, ws.max_row + 1):
        if any(clean_text(ws.cell(row_idx, c).value) for c in cols):
            last = row_idx
    return last
...
class OllamaConfig:
    ...
    def from_env(cls) -> "OllamaConfig":
        ...
        return cls(
            ...
            structured_output=str_to_bool(os.getenv("OLLAMA_STRUCTURED_OUTPUT", "true"), True),
            num_ctx=num_ctx,
...
平台：{clean_text(platform) or '(未指定)'}
MASVS Domain：{clean_text(mastg_domain) or '(共通測項)'}
...
- Android 與 iOS 平台特性不得互相混用；除非目前情況有明確證據，不得因測項名稱自行宣稱使用 Android Keystore、Keychain、ATS、Network Security Configuration、WebView、Jailbreak/Root detection 等機制。
...
RECOMMENDATION_SYSTEM_PROMPT = """你是一位 OWASP MASTG 行動 App 資安檢測報告的改善建議撰寫人員。
你會收到同一個檢測分類或 MASVS domain 中，被人工判定為 low risk 或以上的測項。
...
8. 若分類為 MASVS-STORAGE / CRYPTO / AUTH / NETWORK / PLATFORM / CODE / RESILIENCE，應聚焦該 MASVS domain 與實際觸發測項，不要把無關領域全部列入。
9. Android 與 iOS 平台特性不得混用，且不得把未提供的控制措施寫成產品既有事實。
10. 每種語言輸出一個精簡正式段落，不要標題、Markdown、條列或前言。
11. 僅輸出 JSON object。
...
def collect_category_summaries(ws, header_row: int, headers: dict[str, int], platform: str, last_row: int | None = None) -> list[dict]:
    """依 Static/Dynamic/Fuzzing 或 MASVS domain 彙整，只納入目前平台適用測項。"""
    ...
    for row_idx in range(header_row + 1, last_row + 1):
        raw_test_item = clean_text(ws.cell(row_idx, headers["測項"]).value)
        item_no = clean_text(ws.cell(row_idx, headers["編號"]).value)
        if not item_no:
            heading_domain = extract_masvs_domain(raw_test_item)
            if heading_domain:
                current_domain = heading_domain
            continue
        if not is_relevant_item_id(platform, item_no):
            continue

        test_item = extract_test_item_text(raw_test_item)
        base_category = clean_text(ws.cell(row_idx, category_col).value)
        category = current_domain if re.fullmatch(r"M[AI]\d+", item_no.upper()) and current_domain else base_category
        ...
        summary = categories.setdefault(category, {
            "category": category, "highest_risk": "-", "triggered_items": [],
            "_highest_rank": -1, "_has_pending": False,
        })
        kind = normalize_result_kind(result)
        ...
        if kind in {"low", "medium", "high", "critical"}:
            summary["triggered_items"].append({
                "item_no": item_no, "test_item": test_item, "result": result,
...
def build_category_recommendation_prompt(category: str, triggered_items: list[dict], platform: str = "") -> str:
    lines = []
...
def write_security_recommendation_sheet(wb, summaries: list[dict], recommendation_generate_func: Callable[..., str] | None, platform: str) -> dict:
    ...
    for row_idx, summary in enumerate(summaries, start=2):
        ...
        if triggered and recommendation_generate_func is not None:
            try:
                raw = recommendation_generate_func(category=summary["category"], triggered_items=triggered, platform=platform)
...
def process_workbook(
    *,
    ...
    try:
        detected_platform, sheet_name = detect_platform_and_sheet(wb, platform)
        ws = wb[sheet_name]
        # 若來源檔含 x14 DataValidation，再從原始檔解析並轉為標準格式。
        ...
        last_row = find_last_content_row(ws, header_row, headers)
        ...
        row_context_by_id = {}
        for context_row_idx in range(header_row + 1, last_row + 1):
            context_item_no = clean_text(ws.cell(context_row_idx, headers["編號"]).value).upper()
            if not context_item_no or not is_relevant_item_id(detected_platform, context_item_no):
                continue
            row_context_by_id[context_item_no] = {
                "test_item": extract_test_item_text(ws.cell(context_row_idx, headers["測項"]).value),
        ...
        for row_idx in range(header_row + 1, last_row + 1):
            item_no = clean_text(ws.cell(row_idx, headers["編號"]).value)
            if not item_no or not is_relevant_item_id(detected_platform, item_no):
                continue
            raw_test_item = clean_text(ws.cell(row_idx, headers["測項"]).value)
            test_item = extract_test_item_text(raw_test_item)
            mastg_domain = extract_masvs_domain(raw_test_item)
            result = clean_text(ws.cell(row_idx, headers["判定結果"]).value)
            current_status = clean_text(ws.cell(row_idx, headers["目前情況"]).value)
            remediation = clean_text(ws.cell(row_idx, remediation_col).value) if remediation_col else ""
        ...
        if not normalize_only:
            ...
            for auto_row_idx in range(header_row + 1, last_row + 1):
        ...
        if not normalize_only:
            for format_row_idx in range(header_row + 1, last_row + 1):
                format_output_cells(
                    ws, format_row_idx,
        ...
        if not normalize_only:
            summaries = collect_category_summaries(ws, header_row, headers, detected_platform, last_row)
            recommendation_stats = write_security_recommendation_sheet(
                wb, summaries, recommendation_generate_func, detected_platform
            )
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="AI 自動產生 OWASP MASTG Android / iOS Excel 資安檢測 Report")
    parser.add_argument("excel_path", help="輸入 Excel (.xlsx) 路徑")
    parser.add_argument("--output", help="輸出 Excel 路徑；未指定時自動加上 _report")
    parser.add_argument("--platform", choices=["auto", "android", "ios"], default="auto", help="平台；預設 auto 由工作表名稱辨識")
    parser.add_argument(
        "--normalize-only",
        action="store_true",
    ...
    print("\n完成")
    print(f"平台：{stats['platform']} / 工作表：{stats['sheet_name']}")
